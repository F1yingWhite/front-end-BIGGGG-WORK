...
export function ProductControl() {
  ...
  const fetchProducts = async () => {
    try {
      const response = await axios.get('http://localhost:6001/api/products');
  ...
  const fetchSellers = async () => {
    try {
      const response = await axios.get('http://localhost:6001/api/users/sellers');
  ...
  const handleOk = async () => {
    try {
      ...
      let updatedProducts;
      if (editingProduct) {
        await axios.put(`http://localhost:6001/api/products/${editingProduct.id}`, { ...values, image: mainImage, imageList });
        ...
        await axios.post('http://localhost:6001/api/products', newProduct);
      ...
      <Modal title={editingProduct ? '编辑商品' : '添加商品'} open={addProductVisible} onOk={handleOk} onCancel={handleCancel}>
        <Form form={form} layout="vertical" name="productForm">
          ...
          <Form.Item name="image" label="商品图片" rules={[{ required: true, message: '请上传商品图片!' }]}>
            <Upload
              action="http://localhost:6001/api/upload" // 指定上传路径
